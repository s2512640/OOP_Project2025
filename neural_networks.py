...
#out = encoder_mlp(x) #this is a shortcut for encoder_mlp.__call__(x), witch does the forward pass, its predefined
#mu  = out[:,:latent_dim] #fist half is the mu
#log_var = out[:,latent_dim:] #second half is the log_var
#std = tf.math.exp(0.5*log_var) #std found by log_var
...
#out = encoder_conv(x) #this is a shortcut for encoder_mlp.__call__(x)
#mu  = out[:,:latent_dim]
